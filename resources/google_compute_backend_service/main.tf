resource "google_compute_backend_service" "default" {
  ...
  port_name             = var.backend_type == "instance_groups" ? coalesce(var.named_port, local.default_named_port) : null
  protocol              = coalesce(var.protocol, local.default_backend_protocol)
  health_checks         = var.backend_type == "instance_groups" ? [local.healthcheck] : null
  timeout_sec           = var.backend_type == "instance_groups" ? coalesce(var.backend_timeout, local.default_backend_timeout) : null
  load_balancing_scheme = var.lb_scheme
  dynamic "backend" {
    for_each = var.backend_type == "instance_groups" ? var.instance_groups : []
    content {
  ...
  dynamic "backend" {
    for_each = var.neg_name != null ? [0] : []
    content {
      group = var.region == null ? "projects/${var.project_id}/global/networkEndpointGroups/${var.neg_name}" : "projects/${var.project_id}/regions/${var.region}/networkEndpointGroups/${var.neg_name}"
    }
  }
  dynamic "log_config" {
...
locals {
  healthcheck              = var.healthcheck_name != null ? "projects/${var.project_id}/global/healthChecks/${var.healthcheck_name}" : null
